...
# header-includes:
# -  \usepackage{amsmath}
github-repo: AmeliaMN/ComparingSyntaxForModeling
editor_options: 
...
knit_hooks$set(chunk = function(x, options) {
  ...
  if (!is.null(options$codecap)) {
    x <- paste0('\n<div><span style="display:block;" id = "', options$ref, '"></span>', x, '\n<p class="caption">', options$codecap, '</p></div>\n')
  }
...
allfunctions <- read_excel("data/processed/allfunctions_packages.xlsx") |>
  mutate(section = if_else(section == "tidy", "tidyverse", section)) |>
...
The course these labs accompanied was an introductory statistics course using the Lock5 textbook [@locketal2020]. Weekly lab topics were aligned with lecture content, and standardized across all labs for the semester (13 lab sections using a variety of different software). The topics covered are shown in Table \@ref(tab:topics).
...
Table: (\#tab:topics) List of topics for labs across the 15-week semester.
...
```{r post-sentiment, tab.cap="Responses to the question, ``How was the experience of learning to program in R?''"}
...
x |>
  kable(col.names = c("Section", "Response", "n", "Percent"), booktabs = TRUE, linesep = "", format = "latex") |>
...
loadfonts()
plot_grid(p1, p2, p3, l1, nrow = 4, rel_heights = c(1,1,1.1,0.2), vjust = c(0.9, 1, 1), labels = c("a)", "b)", "c)", NULL), label_fontface = "plain", label_fontfamily = "CM Roman")
```
...
```{r approximate-compute, results='asis', tab.cap= "RStudio Cloud usage statistics. Average use per student per month, broken down by section. To find approximate time per assignment, we have divided each month's average by the number of assignments due in the month. The difference between the sections is also computed, and converted into minutes."}
...
cloud_table <- rstudio_cloud_assignment |>
  select(section, info, n, num) |>
  kable(col.names = c( "", "mean compute time (sd)", "number of \n assignments", "approximate time \n  per assignment"), booktabs = TRUE, format = "html") |>
  pack_rows(index = c("September" = 3, "October" = 3, "November" = 3, "December" = 3)) |>
  column_spec(2:4, width = c("1.2in", "0.9in", "1.5in"))
...
In the formula section, students found frequency tables and relative frequency tables with code as in \@ref(fig:tally-ex1) and \@ref(fig:tally-ex2).
